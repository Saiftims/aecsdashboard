...
function healthForFirm(
  ...
  if (casesSubmitted === 0) return { health: "watch", reason: "Active, no case submission in 30d" };
...
export async function productDashboard(): Promise<ProductDashboardData> {
  ...
  const [sessionHealth, returningRows, accountRows, rawEvents, companiesResult, mappingsResult] =
    await Promise.all([
  ...
  const firms: ProductFirmRow[] = [...merged.values()].map((row) => {
    const status = healthForFirm(row.lastActive, row.submissions, row.failures, now);
    return {
      ...
      casesSubmitted: row.submissions,
  ...
  const activeFirms7d = firms.filter(
    (f) => now.getTime() - new Date(f.lastActive).getTime() <= 7 * DAY,
  ).length;
